...
def arm_and_takeoff(aTargetAltitude):
    ...
    # while not DataStore.vehicle.is_armable:
    #     print(" Waiting for vehicle to initialise...")
    #     time.sleep(1)

    print("Arming motors")
    # Copter should arm in GUIDED mode
    DataStore.vehicle.mode = VehicleMode("GUIDED")
    DataStore.vehicle.armed = True

    # Confirm vehicle armed before attempting to take off
    while not DataStore.vehicle.armed:
        print(" Waiting for arming...")
        time.sleep(1)

    print("Taking off!")
    DataStore.vehicle.simple_takeoff(aTargetAltitude)  # Take off to target altitude

    # t1 = time.time()
    # t2 = time.time()
    # # Wait until the vehicle reaches a safe height before processing the goto
    # #  (otherwise the command after DataStore.vehicle.simple_takeoff will execute
    # #   immediately).
    # while (t2-t1) < 20 :
    #     print(" Altitude: ", DataStore.vehicle.location.global_relative_frame.alt)
    #     # Break and return from function just below target altitude.
    #     if DataStore.vehicle.location.global_relative_frame.alt >= aTargetAltitude * 0.95:
...
def condition_yaw(heading, relative=False):
    ...
    msg = DataStore.vehicle.message_factory.command_long_encode(
    ...
    DataStore.vehicle.send_mavlink(msg)
...
* goto - A convenience function that can use DataStore.vehicle.simple_goto (default) or 
...
def goto_position_target_global_int(aLocation):
    ...
    """
    msg = DataStore.vehicle.message_factory.set_position_target_global_int_encode(
...
def goto_position_target_local_ned(north, east, down):
    ...
    """
    msg = DataStore.vehicle.message_factory.set_position_target_local_ned_encode(
        0,       # time_boot_ms (not used)
        0, 0,    # target system, target component
...
def goto(dNorth, dEast, gotoFunction=DataStore.vehicle.simple_goto):
    """
    Moves the vehicle to a position dNorth metres North and dEast metres East of the current position.

    The method takes a function pointer argument with a single `dronekit.lib.LocationGlobal` parameter for 
    the target position. This allows it to be called with different position-setting commands. 
    By default it uses the standard method: dronekit.lib.DataStore.vehicle.simple_goto().

    The method reports the distance to target every two seconds.
    """
    
    currentLocation = DataStore.vehicle.location.global_relative_frame
    ...
    # while DataStore.vehicle.mode.name=="GUIDED": #Stop action if we are no longer in guided mode.
    #     #print "DEBUG: mode: %s" % DataStore.vehicle.mode.name
    #     remainingDistance=get_distance_metres(DataStore.vehicle.location.global_relative_frame, targetLocation)
